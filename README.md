...
*그림 1: 코어 수별 CPU 사용률 - 코어 수 증가 시 급격한 사용률 감소*

![처리량](image/test1/throughput.png)
*그림 2: 코어 수별 처리량 - 8코어에서 성능 저하, 12코어에서 회복*

![쓰기 실행시간](image/test1/write-time.png)
*그림 3: 코어 수별 쓰기 실행시간 - 처리량과 반비례 관계*

![컴팩션 시간](image/test1/compaction%20time.png)
*그림 4: 코어 수별 컴팩션 시간 - 8코어에서 최악, 12코어에서 최고 성능*

![평균 지연시간](image/test1/average-latency.png)
*그림 5: 코어 수별 평균 지연시간 - 사용자 체감 성능의 비일관성*
...
*그림 6: Subcompactions별 처리량 - subcompactions=2에서 9.4MB/s로 최고 성능*

![쓰기 실행시간](image/test2/write_execution_time.png)
*그림 7: Subcompactions별 쓰기 실행시간 - subcompactions=2에서 590.2초로 최단 시간*

![컴팩션 시간](image/test2/compaction_time.png)
*그림 8: Subcompactions별 컴팩션 시간 - subcompactions=4에서 306초로 최적화*

![CPU 사용률](image/test2/cpu_usage.png)
*그림 9: Subcompactions별 CPU 사용률 - subcompactions=8에서 99.1% 포화 상태*

#### 지연시간 상세 분석 시각화

![P50 지연시간](image/test2/p50.png)
*그림 10: P50 지연시간 (중앙값) - 일반적 사용자 경험, subcompactions=2에서 최적*

![P95 지연시간](image/test2/p95.png)
*그림 11: P95 지연시간 상세 분석 - 상위 5% 사용자 경험, subcompactions=8부터 급격한 악화*

![P99 지연시간](image/test2/p99.png)
*그림 12: P99 지연시간 - 최악 사용자 경험 (1% 사용자가 겪는 지연), subcompactions=32에서 20.7% 악화*
