...
### 1. 가설
- **H1**: `subcompactions=1` 설정에서는 멀티 코어 활용이 제한되어 코어 수가 증가하더라도 성능이 비례하여 향상되지 않을 것입니다.
...
| **쓰기 처리량 (MB/s)** | 9.3 MB/s | 8.1 MB/s | **10.9 MB/s** |
...
![CPU 사용률](image/test1/cpu-usage.png)
*그림 1: **자원 낭비의 심각성** - 12코어 환경에서도 CPU 사용률이 9%에 그쳐 91%의 컴퓨팅 자원이 유휴 상태로 방치됨. 하드웨어 투자 대비 실제 활용도의 극심한 불균형을 보여줌*

![처리량](image/test1/throughput.png)
*그림 2: **성능 예측 불가능성** - 코어 수 증가가 성능 향상을 보장하지 않음을 입증. 4→8코어에서 처리량이 9.3→8.1MB/s로 13% 감소 후, 8→12코어에서 8.1→10.9MB/s로 35% 증가하는 비일관적 패턴*

![쓰기 실행시간](image/test1/write-time.png)
*그림 3: **작업 완료 시간의 비효율성** - 8코어에서 682초로 최악의 성능을 보인 후 12코어에서 505초로 급격히 개선. 코어 증가에 따라 비일관적인 패턴을 보임*

![컴팩션 시간](image/test1/compaction%20time.png)
*그림 4: **컴팩션 작업의 스케일링 한계** - 기본 설정에서는 컴팩션이 단일 스레드로 동작하여 8코어에서 499초의 최악 성능 기록. 12코어에서 362초로 최고의 성능 기록. 비일관적인 패턴을 보임*

![평균 지연시간](image/test1/average-latency.png)
*그림 5: **사용자 경험의 불안정성** - 8코어에서 평균 지연시간이 13.6µs로 악화되었다가 12코어에서 10.1µs로 개선. 하드웨어 확장만으로는 일관된 사용자 경험 보장 불가능*
...
![처리량](image/test2/throughput.png)
*그림 6: **전체 시스템 성능의 최적점** - subcompactions=2에서 9.4MB/s로 최고 처리량 달성 후 병렬도 증가 시 지속적 저하. subcompactions=32에서는 7.7MB/s로 최적점 대비 18% 감소*

![쓰기 실행시간](image/test2/write_execution_time.png)
*그림 7: **작업 완료 시간의 최적화** - subcompactions=2에서 590.2초로 최단 완료 시간 기록. 병렬도가 증가할수록 작업 시간이 선형적으로 증가하여 722.4초(subcompactions=32)까지 악화되는 명확한 성능 저하 패턴*

![컴팩션 시간](image/test2/compaction_time.png)
*그림 8: **컴팩션 성능** - 순수 컴팩션 속도는 subcompactions=4에서 306초로 최적화되었음*

![CPU 사용률](image/test2/cpu_usage.png)
*그림 9: **CPU 활용률** - subcompactions=8에서 99.1% 포화 상태 도달하지만 실제 처리량은 8.1MB/s로 최적점(9.4MB/s) 대비 14% 저하. 높은 CPU 사용률이 좋은 성능을 보장하지 않음을 보여주었음*

#### 지연시간 상세 분석 시각화

![P50 지연시간](image/test2/p50.png)
*그림 10: **일반 사용자 경험 최적화** - P50 지연시간(중앙값)이 subcompactions=2에서 8.598µs로 최저치 기록. 50%의 일반 사용자가 체감하는 응답 속도가 적정 병렬화에서 최적화됨을 보여줌*

![P95 지연시간](image/test2/p95.png)
*그림 11: **상위 사용자 경험의 임계점** - P95 지연시간이 subcompactions=2에서 14.533µs로 최적화 후, subcompactions=8부터 15.552µs로 급격히 악화. 상위 5% 사용자 경험에서도 과도한 병렬화의 부정적 영향이 나타남*

![P99 지연시간](image/test2/p99.png)
*그림 12: **극단 사용자 경험의 악화** - P99 지연시간이 subcompactions=32에서 24.62µs로 최적점(20.391µs) 대비 20.7% 악화. 최악 사용자 경험(상위 1%)에서 병렬화 부작용이 가장 극명하게 드러남*
...
- **적정 병렬화의 중요성**: `subcompactions=2`에서 전체 성능이 최고점에 도달한 후, `subcompactions=4`를 넘어서면서 성능이 급격히 저하되었습니다. 특히 `subcompactions=8` 이상에서는 처리량이 지속적으로 감소하여 `subcompactions=32`에서는 처리량이 7.7MB/s로 최적점 대비 18%나 떨어졌습니다. 이는 **2~4 정도의 적정 수준에서 전체 성능이 최적화**됨을 시사합니다.
...
- **결론**: 4코어 환경에서 최적의 **전체 시스템 성능은 `subcompactions=2`**에서, **컴팩션 단일 작업 성능은 `subcompactions=4`**에서 달성되었습니다. 코어수와 비슷한 수로 설정 (**2~4 정도의 적정 수준으로 설정**)하는 것이 RocksDB 성능 최적화의 핵심 원칙입니다.
...
**둘째, 최적의 병렬 컴팩션 성능은 코어수와 비슷한 수준의 병렬도에서 달성됩니다.** 4코어 환경에서 진행한 실험 결과, 전체 시스템 성능(처리량 9.4MB/s, P99 지연시간 20.39µs)은 `subcompactions=2`에서 최고치를 기록했습니다. 과도한 병렬화(`subcompactions=8` 이상)는 컨텍스트 스위칭 비용만 증가시켜 오히려 성능을 14% 이상 저하시켰습니다. 특히 CPU 사용률이 99%에 달해도 실제 처리량은 감소하는 '병렬화의 역설'을 확인했으며, **2~4 정도의 적정 범위에서 최적 성능**이 나타났습니다.
